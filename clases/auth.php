...
class Auth {
  ...
  protected function renovarRemember($posicion,$rnd) {
  ...
  protected function crearRemember($rnd) {
    $nuevalinea = [];
  ...
  public function comprobarRemember(){
    ...
    if ($archivo) {
      while (($linea = fgets($archivo)) !== false) {
        $linea = json_decode($linea, true);
        if ($_SERVER['REMOTE_ADDR'] == $linea['ip']) {
          $this->renovarRemember($cont, $random);
          $band = 1;
          break;
        }
        else {
  ...
  public function usuarioLogueado(db $db) {
    if ($this->estaLogueado()) {
      ...
      return $db->traerPorUsuario($usuario);
    }
    else {
      return NULL;
    }
